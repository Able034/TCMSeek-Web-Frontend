      ...
      <div class="graph-controls card">
        ...
        <div v-if="selectedCenterId && searchKeyword" class="selection-hint">
          <el-tag size="small" type="success">
            ...
            已选择: {{ searchKeyword }} ({{ selectedCenterId }})
          </el-tag>
        </div>
...
import { buildKnowledgeGraph, searchCenterNodes } from '../api/medicine'
...
const handleCenterTypeChange = () => {
  ...
  // 默认选中所有关系
  relationFilters.value = centerType.value ? getDefaultRelationSelection(centerType.value) : []
}
...
const buildGraph = async () => {
  ...
  try {
    console.log('构建图谱 - centerType:', centerType.value, 'centerId:', selectedCenterId.value, 'relations:', relationFilters.value)

    const res = await buildKnowledgeGraph({
      centerType: centerType.value,
      centerId: selectedCenterId.value,
      relations: relationFilters.value.join(',')
    })
    ...
    if (res.code === 200 && res.data) {
      ...
      if (graphData.value.nodes.length === 0) {
        ...
        ElMessage.success(t('graph.buildSuccess') || `图谱构建成功！节点: ${res.data.nodeCount}, 边: ${res.data.edgeCount}`)
      }
    } else {
      ElMessage.error(res.msg || t('graph.buildFailed') || '构建图谱失败')
      graphData.value = { nodes: [], links: [] }
    }
  } catch (error) {
    console.error('构建图谱失败:', error)
    ElMessage.error(t('graph.buildError') || '构建图谱时发生错误')
    graphData.value = { nodes: [], links: [] }
  } finally {
    loading.value = false
  }
...
const handleNodeDoubleClick = (nodeData) => {
  ...
  // 更新关系筛选（默认选中所有）
  relationFilters.value = getDefaultRelationSelection(newCenterType)
