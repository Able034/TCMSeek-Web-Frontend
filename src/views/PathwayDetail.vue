<template>
  <div class="pathway-detail-page">
    <div class="container">
      ...
      <!-- 内容区域 -->
        <div v-else-if="pathway" class="content-wrapper">
          <!-- 基本信息卡片 -->
          <div class="card detail-header" id="pathway-overview">
          <div class="header-icon">
            <el-icon><WindPower /></el-icon>
          </div>
          <div class="header-content">
            <h1 class="pathway-name">{{ pathway.name }}</h1>
            <div class="pathway-meta">
              <span class="meta-item">
                <el-icon><Ticket /></el-icon>
                <strong>{{ $t('medicine.pathway.id') }}:</strong>
                {{ pathway.pathwayId }}
              </span>
              <span class="meta-item">
                <el-icon><Document /></el-icon>
                <strong>{{ $t('medicine.pathway.source') }}:</strong>
                {{ pathway.source || 'KEGG' }}
              </span>
            </div>
            <div class="external-links">
              <a 
                :href="`https://www.kegg.jp/entry/${pathway.pathwayId}`" 
                target="_blank"
              ...
              <a 
                :href="`https://www.pathwaycommons.org/pc12/search?q=${pathway.pathwayId}`" 
                target="_blank"
                rel="noopener noreferrer"
                class="external-link"
              >
        ...
        <!-- 统计信息卡片 -->
          <div class="card statistics-card" id="pathway-stats">
          <h2>{{ $t('medicine.pathway.statistics') }}</h2>
          <div class="stats-grid">
            <div class="stat-item">
              ...
              <div class="stat-content">
                ...
                <div class="stat-label">{{ $t('medicine.pathway.relatedGenes') }}</div>
              </div>
            </div>
            <div class="stat-item">
              <div class="stat-icon">
              ...
              <div class="stat-content">
                <div class="stat-value">{{ statistics.compoundCount || 0 }}</div>
                <div class="stat-label">{{ $t('medicine.pathway.relatedCompounds') }}</div>
              </div>
            </div>
            <div class="stat-item">
              <div class="stat-icon">
              ...
              <div class="stat-content">
                <div class="stat-value">{{ statistics.herbCount || 0 }}</div>
                <div class="stat-label">{{ $t('medicine.pathway.relatedHerbs') }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Tabs 选项卡 -->
          <div class="card tabs-card" id="pathway-related">
          <el-tabs v-model="activeTab" @tab-click="handleTabClick">
            <!-- 关联靶标/基因 Tab -->
            <el-tab-pane 
              :label="$t('medicine.pathway.relatedTargets')" 
              name="targets"
            >
              <div class="tab-content">
                ...
                <div class="search-bar-inline">
                  <el-input
                    ...
                    :placeholder="$t('medicine.pathway.searchTargets')"
                    clearable
                    @clear="loadTargets"
            ...
            <!-- 影响此通路的化合物 Tab -->
            <el-tab-pane 
              :label="$t('medicine.pathway.relatedCompounds')" 
              name="compounds"
            >
              <div class="tab-content">
                <!-- 搜索框 -->
                <div class="search-bar-inline">
                  <el-input
                    v-model="compoundsKeyword"
                    :placeholder="$t('medicine.pathway.searchCompounds')"
                    clearable
                    @clear="loadCompounds"
                    @keyup.enter="loadCompounds"
                  >
                    <template #prefix>
                      <el-icon><Search /></el-icon>
                    </template>
                  </el-input>
                  <el-button type="primary" @click="loadCompounds">
                    {{ $t('common.search') }}
                  </el-button>
                </div>

                <!-- 加载状态 -->
                <div v-if="compoundsLoading" class="loading-state">
                  <el-icon class="loading-icon"><Loading /></el-icon>
                  <p>{{ $t('common.loading') }}...</p>
                </div>

                <!-- 空数据 -->
                <div v-else-if="compounds.length === 0" class="empty-state">
                  <el-icon class="empty-icon"><Document /></el-icon>
                  <p>{{ $t('common.noData') }}</p>
                </div>
                ...
                <div v-else class="related-table-wrapper">
                  <table class="related-table">
                    <thead>
                      <tr>
                        <th>InChIKey</th>
                        <th>{{ $t('medicine.compound.molecularFormula') }}</th>
                        <th>{{ $t('medicine.compound.molecularWeight') }}</th>
                        <th>{{ $t('medicine.compound.pubchemCid') }}</th>
                        <th>{{ $t('medicine.pathway.targetCount') }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr 
                        ...
                        @click="goToCompoundDetail(compound.inchikey)"
                        class="clickable-row"
                      >
                        <td class="id-cell">{{ compound.inchikey }}</td>
                        ...
                        <td>
                          <span class="count-badge">{{ compound.targetCount || 0 }}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  <div class="pagination-wrapper">
                    <el-pagination
                      ...
                      :page-size="compoundsPagination.pageSize"
            ...
            <!-- 影响此通路的中药 Tab -->
            <el-tab-pane 
              :label="$t('medicine.pathway.relatedHerbs')" 
              name="herbs"
            >
              <div class="tab-content">
                <!-- 搜索框 -->
                <div class="search-bar-inline">
                  <el-input
                    v-model="herbsKeyword"
                    :placeholder="$t('medicine.pathway.searchHerbs')"
                    clearable
                    @clear="loadHerbs"
                    @keyup.enter="loadHerbs"
                  >
                    <template #prefix>
                      <el-icon><Search /></el-icon>
                    </template>
                  </el-input>
                  <el-button type="primary" @click="loadHerbs">
                    {{ $t('common.search') }}
                  </el-button>
                </div>

                <!-- 加载状态 -->
                <div v-if="herbsLoading" class="loading-state">
                  <el-icon class="loading-icon"><Loading /></el-icon>
                  <p>{{ $t('common.loading') }}...</p>
                </div>

                <!-- 空数据 -->
                <div v-else-if="herbs.length === 0" class="empty-state">
                  <el-icon class="empty-icon"><Document /></el-icon>
                  <p>{{ $t('common.noData') }}</p>
                </div>
                ...
                <div v-else class="related-table-wrapper">
                  <table class="related-table">
                    <thead>
                      <tr>
                        ...
                        <th>{{ $t('medicine.pathway.compoundCount') }}</th>
                        <th>{{ $t('medicine.pathway.targetCount') }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr 
                        v-for="herb in herbs" 
                        :key="herb.tcmHerbId"
                        @click="goToHerbDetail(herb.tcmHerbId)"
                        class="clickable-row"
                      >
                        <td class="id-cell">{{ herb.tcmHerbId }}</td>
                        <td class="name-cell">
                          <strong>{{ herb.herbNameZh || '-' }}</strong>
                        </td>
                        <td>{{ herb.pinyinName || '-' }}</td>
                        <td>
                          <span class="count-badge">{{ herb.compoundCount || 0 }}</span>
                        </td>
                        <td>
                          <span class="count-badge">{{ herb.targetCount || 0 }}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  <div class="pagination-wrapper">
                    <el-pagination
                      ...
                      :page-size="herbsPagination.pageSize"
            ...
            <!-- 知识图谱 Tab -->
            <el-tab-pane 
              :label="$t('medicine.pathway.knowledgeGraph')" 
              name="graph"
            >
              <div class="tab-content">
...
import {
  ArrowLeft, Loading, WindPower, Ticket, Document, Link,
  Finished, CollectionTag, Orange, Search, WarnTriangleFilled, Share
...
const anchorSections = computed(() => {
  ...
  return [
    { id: 'pathway-overview', labelKey: 'common.anchorOverview' },
    { id: 'pathway-stats', labelKey: 'medicine.pathway.statistics' },
    { id: 'pathway-related', labelKey: 'medicine.pathway.relatedData' }
  ]
})
...
const compoundsKeyword = ref('')
const compoundsPagination = ref({ page: 1, pageSize: 10, total: 0 })

// 影响此通路的中药
const herbs = ref([])
const herbsLoading = ref(false)
const herbsKeyword = ref('')
const herbsPagination = ref({ page: 1, pageSize: 10, total: 0 })
...
// 加载通路基本信息
const loadPathwayDetail = async () => {
  loading.value = true
  try {
    const response = await getPathwayDetail(pathwayId.value)
    if (response.code === 200) {
      pathway.value = response.data
    } else {
      ElMessage.error(response.msg || t('common.loadFailed'))
    }
  } catch (error) {
    console.error('加载通路详情失败:', error)
...
const loadStatistics = async () => {
  try {
    const response = await getPathwayStatistics(pathwayId.value)
    if (response.code === 200) {
...
const loadTargets = async () => {
  ...
  try {
    const response = await getPathwayTargets(pathwayId.value, {
...
const loadCompounds = async () => {
  ...
  try {
    const response = await getPathwayCompounds(pathwayId.value, {
      page: compoundsPagination.value.page,
      pageSize: compoundsPagination.value.pageSize,
      keyword: compoundsKeyword.value
    })
    if (response.code === 200) {
...
const loadHerbs = async () => {
  ...
  try {
    const response = await getPathwayHerbs(pathwayId.value, {
      page: herbsPagination.value.page,
      pageSize: herbsPagination.value.pageSize,
      keyword: herbsKeyword.value
    })
    if (response.code === 200) {
...
const loadGraphData = async () => {
  ...
  try {
    const response = await getPathwayGraph(pathwayId.value)
    if (response.code === 200) {
...
const handleTabClick = (tab) => {
  ...
  if (tabName === 'targets' && targets.value.length === 0) {
    loadTargets()
  } else if (tabName === 'compounds' && compounds.value.length === 0) {
    loadCompounds()
